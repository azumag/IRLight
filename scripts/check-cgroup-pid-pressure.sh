...
current_path="${1:-${IRLIGHT_CGROUP_PIDS_CURRENT_PATH:-/sys/fs/cgroup/pids.current}}"
maximum_path="${2:-${IRLIGHT_CGROUP_PIDS_MAX_PATH:-/sys/fs/cgroup/pids.max}}"
warning_percent="${IRLIGHT_CGROUP_PIDS_WARNING_PERCENT:-80}"
critical_percent="${IRLIGHT_CGROUP_PIDS_CRITICAL_PERCENT:-90}"
...
if [[ "$maximum_raw" == "max" ]]; then
  printf 'IRLIGHT_CGROUP_PID_PRESSURE status=OK usage_percent=NA current=%s maximum=max warning_percent=%s critical_percent=%s\n' \
    "$current" \
    "$warning_percent" \
    "$critical_percent"
  exit 0
...
pressure_usage_percent "$current" "$maximum" || unknown invalid_pid_values
...
printf 'IRLIGHT_CGROUP_PID_PRESSURE status=%s usage_percent=%s current=%s maximum=%s warning_percent=%s critical_percent=%s\n' \
  "$status" \
  "$usage_percent" \
  "$current" \
  "$maximum" \
