...
tmp_dir="$(mktemp -d)"
override="$tmp_dir/rtmp-netem-matrix.override.yml"
...
publisher_name="irlight-netem-matrix-publisher-$RANDOM-$RANDOM"
publisher_log="$tmp_dir/publisher.log"
publisher_pid=""
netem_applied=0
email="rtmp-netem-matrix-$(date +%s)-$RANDOM@example.invalid"
...
run_profile() {
  ...
  while (( SECONDS < deadline )); do
    ...
    printf 'profile=%s session_status=%s relay=online\n' "$profile" "$status"
    sleep 2
  done
  ...
  printf 'profile=%s result=PASS observed_hold=%s %s\n' "$profile" "$hold_summary" "$event_summary"
}
...
curl -fsS --max-time 10 -X POST "$base_url/v1/auth/register" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"$email\",\"password\":\"$password\",\"display_name\":\"RTMP Netem Matrix Smoke\"}" >/dev/null
login
...
prepared="$(curl -fsS --max-time 10 -b "$cookie_jar" -X POST \
  ...
  -H 'Content-Type: application/json' \
  -H "X-CSRF-Token: $csrf" \
  -H "Idempotency-Key: rtmp-netem-matrix-$session_id" \
...
create_publisher_container
start_publisher
stage "publisher supervisor started"
wait_session_status LIVE 60
wait_continuity_live 60
assert_output_relay_online
...
stage "all selected netem degradation profiles passed"
echo "RTMP netem degradation matrix passed profiles=$selected_profiles duration=${profile_seconds}s"
