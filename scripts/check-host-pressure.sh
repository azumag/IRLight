...
if (( disk_code == 2 || memory_code == 2 || load_code == 2 || psi_code == 2 || file_handle_code == 2 || conntrack_code == 2 )); then
  status="CRITICAL"
  exit_code=2
elif (( disk_code == 3 || memory_code == 3 || load_code == 3 || psi_code == 3 || file_handle_code == 3 || conntrack_code == 3 )); then
  status="UNKNOWN"
  exit_code=3
elif (( disk_code == 1 || memory_code == 1 || load_code == 1 || psi_code == 1 || file_handle_code == 1 || conntrack_code == 1 )); then
  status="WARNING"
  exit_code=1
fi

printf 'IRLIGHT_HOST_PRESSURE status=%s disk_status=%s memory_status=%s load_status=%s psi_status=%s file_handle_status=%s conntrack_status=%s\n' \
  "$status" \
  "$disk_status" \
  "$memory_status" \
  "$load_status" \
