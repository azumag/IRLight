...
executed.  ``apply`` is deliberately harder to invoke: callers must provide a
bounded duration and an explicit disruption acknowledgement.  Applied qdiscs
are removed in ``finally`` so a test interruption does not intentionally leave
the requested fault behind.

This tool intentionally covers only qdisc-local packet loss, latency/jitter,
and complete packet blackholes. DNS, route, firewall, and TCP reset injection
remain separate test concerns because their cleanup and blast radius differ.
"""
...
def build_fault_plan(
    *,
    ...
    if disconnect and (loss_percent is not None or latency_ms is not None or jitter_ms is not None):
    ...
    argv = ["tc", "qdisc", "replace", "dev", interface, "root", "netem"]
    if disconnect:
    ...
    cleanup = ("tc", "qdisc", "del", "dev", interface, "root")
    return FaultPlan(
        interface=interface,
...
def _add_profile_arguments(parser: argparse.ArgumentParser, *, duration_required: bool) -> None:
    ...
    group.add_argument("--loss", type=int, choices=LOSS_PERCENT_CHOICES, dest="loss_percent")
    group.add_argument("--disconnect", action="store_true")
    parser.add_argument("--latency", type=int, choices=LATENCY_MS_CHOICES, dest="latency_ms")
...
def _parser() -> argparse.ArgumentParser:
    ...
    apply_parser = subparsers.add_parser(
        "apply",
        help="apply a bounded fault and always attempt cleanup afterwards",
    )
    _add_profile_arguments(apply_parser, duration_required=True)
    apply_parser.add_argument(
        "--confirm-network-disruption",
        action="store_true",
        required=True,
        help="acknowledge that the selected interface will be disrupted",
    )

    clear = subparsers.add_parser("clear", help="remove the root qdisc from an interface")
    ...
    clear.add_argument(
        ...
        action="store_true",
        required=True,
        help="acknowledge that qdisc state will be changed",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if cleanup_error is not None:
            raise RuntimeError(f"network fault cleanup failed: {cleanup_error}") from cleanup_error
