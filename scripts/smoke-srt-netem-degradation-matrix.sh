...
continuity_state_matches() {
  local expected_status="$1" expected_video="$2" expected_audio="$3" payload
...
create_publisher_container() {
  ...
  docker exec "$publisher_name" gst-inspect-1.0 mpegtsmux >/dev/null
}
...
streamid=f"publish:live/input:{user}:{password}"
uri=f"srt://mediamtx:8890?mode=caller&streamid={streamid}"
description=f'''mpegtsmux name=mux ! srtsink uri="{uri}"
...
while True:
    ...
    if started == Gst.StateChangeReturn.FAILURE:
        print(f"SRT publisher attempt {attempt} failed to start; retrying",flush=True)
        pipeline.set_state(Gst.State.NULL)
        time.sleep(1)
        continue
    print(f"SRT publisher attempt {attempt} started",flush=True)
    try:
        while True:
            ...
            if message.type == Gst.MessageType.ERROR:
                ...
                print(f"SRT publisher attempt {attempt} error: {err}; {debug}; retrying",flush=True)
            else:
                print(f"SRT publisher attempt {attempt} reached EOS; retrying",flush=True)
            break
    finally:
        pipeline.set_state(Gst.State.NULL)
    time.sleep(1)
...
if not holds: print("none")
    ...
    latest=holds[-1]; print("{}|{}".format(latest.get("sequence"),latest.get("reason_code") or "UNKNOWN"))
' "$after_sequence"
}
...
if fatal: raise SystemExit(f"fatal Session event during netem profile: {fatal!r}")
holds=[e for e in events if e.get("type")=="session.holding"]
if holds:
    last_hold=holds[-1]
    recovered=next((e for e in events if int(e.get("sequence",0))>int(last_hold.get("sequence",0)) and e.get("type")=="session.recovered"),None)
    if recovered is None: raise SystemExit(f"Session held but did not recover after profile: {events!r}")
...
run_profile() {
  local profile="$1" baseline_sequence deadline status hold_summary event_summary
  local -a args=()
  ...
  while (( SECONDS < deadline )); do
    assert_output_relay_online
    kill -0 "$publisher_pid" 2>/dev/null || { echo "publisher supervisor exited during profile=$profile" >&2; return 1; }
  ...
  wait_continuity_live 90
  assert_output_relay_online
  kill -0 "$publisher_pid" 2>/dev/null || { echo "publisher supervisor exited before LIVE recovery for profile=$profile" >&2; return 1; }
...
[[ -n "$provider_server_id" && "$provider_server_id" != "None" ]] || { echo "prepare did not allocate provider_server_id" >&2; exit 1; }
...
(( ${#profiles[@]} > 0 )) || { echo "NETEM_MATRIX_PROFILES must contain at least one profile" >&2; exit 2; }
