...
cgroup_dir="${1:-${IRLIGHT_CGROUP_PSI_DIR:-}}"
some_warning_percent="${IRLIGHT_CGROUP_PSI_SOME_WARNING_PERCENT:-25}"
some_critical_percent="${IRLIGHT_CGROUP_PSI_SOME_CRITICAL_PERCENT:-50}"
full_warning_percent="${IRLIGHT_CGROUP_PSI_FULL_WARNING_PERCENT:-5}"
full_critical_percent="${IRLIGHT_CGROUP_PSI_FULL_CRITICAL_PERCENT:-20}"
...
for resource in cpu memory io; do
  if [[ ! -r "$cgroup_dir/$resource.pressure" ]]; then
    unknown psi_unavailable
  fi
done

if ! cpu_some="$(read_avg10 "$cgroup_dir/cpu.pressure" some)"; then
  unknown invalid_cpu_psi
fi
if ! memory_some="$(read_avg10 "$cgroup_dir/memory.pressure" some)" || \
   ! memory_full="$(read_avg10 "$cgroup_dir/memory.pressure" full)"; then
  unknown invalid_memory_psi
fi
if ! io_some="$(read_avg10 "$cgroup_dir/io.pressure" some)" || \
   ! io_full="$(read_avg10 "$cgroup_dir/io.pressure" full)"; then
...
printf 'IRLIGHT_CGROUP_PSI_PRESSURE status=%s cpu_some_avg10=%s memory_some_avg10=%s memory_full_avg10=%s io_some_avg10=%s io_full_avg10=%s some_warning_percent=%s some_critical_percent=%s full_warning_percent=%s full_critical_percent=%s\n' \
  "$status" "$cpu_some" "$memory_some" "$memory_full" "$io_some" "$io_full" \
  "$some_warning_percent" "$some_critical_percent" "$full_warning_percent" "$full_critical_percent"
exit "$exit_code"
