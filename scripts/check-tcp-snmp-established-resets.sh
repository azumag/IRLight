...
unknown() {
  local reason="$1"
  printf 'IRLIGHT_TCP_SNMP_ESTABLISHED_RESETS status=UNKNOWN reason=%s\n' "$reason"
  exit 3
}
...
read_tcp_counter() {
  ...
  local -a values=()
  local i key value
  local seen_established_resets=0
  ...
  for ((i = 1; i < ${#headers[@]}; i++)); do
    ...
    if [[ "$key" == "EstabResets" ]]; then
      (( seen_established_resets == 0 )) || unknown invalid_snmp_record
      is_uint64_bounded "$value" || unknown invalid_snmp_record
      seen_established_resets=1
      printf -v "${prefix}_established_resets" '%d' "$((10#$value))"
    fi
  done

  (( seen_established_resets == 1 )) || unknown invalid_snmp_record
}
...
if (( current_established_resets < baseline_established_resets )); then
  unknown counter_reset
fi

delta_established_resets="$((current_established_resets - baseline_established_resets))"

status="OK"
reason="none"
exit_code=0
if (( delta_established_resets > 0 )); then
  status="WARNING"
  reason="tcp_established_reset_activity"
  exit_code=1
fi

printf 'IRLIGHT_TCP_SNMP_ESTABLISHED_RESETS status=%s reason=%s established_resets_delta=%s\n' \
  "$status" \
  "$reason" \
  "$delta_established_resets"
exit "$exit_code"
