"""Bounded, non-blocking reads for Egress Gateway runtime secret files.

The configured paths are operator-controlled rather than public request input,
but secret readers still need a small fail-closed boundary: a mistaken
FIFO/device must not block the Egress Gateway, oversized files must not be read
without bound, and diagnostics must not expose secret paths or raw OS errors.
