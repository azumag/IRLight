...
class NetworkEgressTcpAttemptFailsTest(unittest.TestCase):
    def run_check(
        self,
        *,
        mode: str | None = None,
        ...
        with tempfile.TemporaryDirectory(prefix="irlight-network-egress-attempt-fails-") as tmp:
            root = Path(tmp)
            scripts = root / "scripts"
            scripts.mkdir()
            ...
            for name in (
                ...
                target = scripts / name
                if name == "check-tcp-snmp-attempt-fails.sh" and attempt_checker_body is not None:
                    target.write_text(attempt_checker_body, encoding="utf-8")
                ...
                elif name == "check-tcp-snmp-established-resets.sh" and established_checker_body is not None:
            ...
            if mode is not None:
                env["IRLIGHT_TCP_ATTEMPT_FAILS_MODE"] = mode
    ...
    def test_enabled_ok_and_warning_report_attempt_fail_field(self) -> None:
        ...
        self.assertEqual(
            ok.stdout.strip(),
            "IRLIGHT_NETWORK_EGRESS_HEALTH status=OK link_status=OK "
            "ipv4_route_status=OK ipv6_route_status=OK tcp_attempt_fails_status=OK family=dual",
        )

        warning, _ = self.run_check(
            mode="enabled",
            current=snmp_record(AttemptFails=11),
            baseline=baseline,
        )
        self.assertEqual(warning.returncode, 1)
        self.assertIn("status=WARNING", warning.stdout)
        self.assertIn("tcp_attempt_fails_status=WARNING", warning.stdout)

    def test_missing_baseline_and_invalid_mode_fail_closed(self) -> None:
        missing, _ = self.run_check(mode="enabled")
        self.assertEqual(missing.returncode, 3)
        self.assertIn("tcp_attempt_fails_status=UNKNOWN", missing.stdout)

        invalid, _ = self.run_check(mode="yes")
        self.assertEqual(invalid.returncode, 3)
        self.assertIn("tcp_attempt_fails_status=UNKNOWN", invalid.stdout)

    def test_confirmed_route_critical_wins_over_attempt_fail_warning_or_unknown(self) -> None:
        warning, _ = self.run_check(
            mode="enabled",
            current=snmp_record(AttemptFails=1),
        ...
        self.assertIn("tcp_attempt_fails_status=UNKNOWN", unknown.stdout)
    ...
    def test_all_optional_components_have_stable_order_and_severity(self) -> None:
        ...
        self.assertEqual(
            ...
            "tcp_attempt_fails_status=WARNING family=dual",
        )

    def test_attempt_fail_component_timeout_becomes_unknown(self) -> None:
        ...
        self.assertIn("tcp_attempt_fails_status=UNKNOWN", result.stdout)
