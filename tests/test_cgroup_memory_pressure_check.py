...
SCRIPT = ROOT / "scripts" / "check-cgroup-memory-pressure.sh"


class CgroupMemoryPressureCheckTest(unittest.TestCase):
    def _run(
        self,
        current: str = "100\n",
        maximum: str = "1000\n",
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="irlight-cgroup-memory-pressure-") as temporary:
            root = Path(temporary)
    ...
    def test_over_limit_is_critical(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_MEMORY_PRESSURE status=CRITICAL usage_percent=OVER_LIMIT current_bytes=101 maximum_bytes=100 warning_percent=80 critical_percent=90",
        )
    ...
    def test_zero_limit_is_valid_and_critical(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_MEMORY_PRESSURE status=CRITICAL usage_percent=NO_HEADROOM current_bytes=0 maximum_bytes=0 warning_percent=80 critical_percent=90",
        )
    ...
    def test_unlimited_local_limit_is_ok(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_MEMORY_PRESSURE status=OK usage_percent=NA current_bytes=123 maximum_bytes=max warning_percent=80 critical_percent=90",
        )
    ...
    def test_thresholds_are_decimal_and_configurable(self) -> None:
        result = self._run(
            ...
            "100\n",
            env_overrides={
                "IRLIGHT_CGROUP_MEMORY_WARNING_PERCENT": "080",
                "IRLIGHT_CGROUP_MEMORY_CRITICAL_PERCENT": "090",
            },
        )
        self.assertEqual(result.returncode, 1)
    ...
    def test_invalid_threshold_is_unknown(self) -> None:
        result = self._run(
            env_overrides={"IRLIGHT_CGROUP_MEMORY_WARNING_PERCENT": "101"}
        )
        self.assertEqual(result.returncode, 3)
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_MEMORY_PRESSURE status=UNKNOWN reason=invalid_threshold",
        )
    ...
    def test_malformed_current_is_unknown(self) -> None:
        for content in (
            ...
            with self.subTest(content=content):
                ...
                self.assertEqual(
                    result.stdout.strip(),
                    "IRLIGHT_CGROUP_MEMORY_PRESSURE status=UNKNOWN reason=invalid_current",
                )
    ...
    def test_malformed_maximum_is_unknown(self) -> None:
        for content in (
            ...
            with self.subTest(content=content):
                ...
                self.assertEqual(
                    result.stdout.strip(),
                    "IRLIGHT_CGROUP_MEMORY_PRESSURE status=UNKNOWN reason=invalid_maximum",
                )
