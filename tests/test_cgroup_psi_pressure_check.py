...
SCRIPT = ROOT / "scripts" / "check-cgroup-psi-pressure.sh"


class CgroupPsiPressureCheckTest(unittest.TestCase):
    def _run(
        self,
        *,
        cpu_some: str = "1.00",
        ...
        with tempfile.TemporaryDirectory(prefix="irlight-cgroup-psi-pressure-") as temporary:
            cgroup_dir = Path(temporary)
            ...
            (cgroup_dir / "io.pressure").write_text(
            ...
            return subprocess.run(
                ["bash", str(SCRIPT), str(cgroup_dir)],
                env=env,
                text=True,
                capture_output=True,
                check=False,
    ...
    def test_thresholds_are_decimal_and_configurable(self) -> None:
        result = self._run(
            cpu_some="9.50",
            env_overrides={
                "IRLIGHT_CGROUP_PSI_SOME_WARNING_PERCENT": "010",
                "IRLIGHT_CGROUP_PSI_SOME_CRITICAL_PERCENT": "020",
                "IRLIGHT_CGROUP_PSI_FULL_WARNING_PERCENT": "001",
                "IRLIGHT_CGROUP_PSI_FULL_CRITICAL_PERCENT": "010",
            },
        )
        self.assertEqual(result.returncode, 0)
    ...
    def test_invalid_threshold_is_unknown(self) -> None:
        result = self._run(
            env_overrides={"IRLIGHT_CGROUP_PSI_SOME_WARNING_PERCENT": "101"}
        )
        self.assertEqual(result.returncode, 3)
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_PSI_PRESSURE status=UNKNOWN reason=invalid_threshold",
        )
    ...
    def test_malformed_cpu_psi_is_unknown(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_PSI_PRESSURE status=UNKNOWN reason=invalid_cpu_psi",
        )
    ...
    def test_target_is_required(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_CGROUP_PSI_PRESSURE status=UNKNOWN reason=target_required",
        )
