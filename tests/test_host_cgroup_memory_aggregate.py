...
CGROUP_WRAPPER = ROOT / "scripts" / "check-host-cgroup-memory-pressure.sh"
CGROUP_CHECKER = ROOT / "scripts" / "check-cgroup-memory-pressure.sh"
SCALAR_HELPER = ROOT / "scripts" / "lib" / "scalar-pressure-common.sh"


class HostCgroupMemoryAggregateTest(unittest.TestCase):
    def _run(
        self,
        *,
        mode: str | None = None,
        ...
        with tempfile.TemporaryDirectory(prefix="irlight-host-cgroup-memory-aggregate-") as temporary:
            root = Path(temporary)
            scripts = root / "scripts"
            lib = scripts / "lib"
            lib.mkdir(parents=True)
            shutil.copyfile(SCRIPT, scripts / "check-host-pressure.sh")
            shutil.copyfile(CGROUP_WRAPPER, scripts / "check-host-cgroup-memory-pressure.sh")
            shutil.copyfile(CGROUP_CHECKER, scripts / "check-cgroup-memory-pressure.sh")
            ...
            if mode is not None:
                env["IRLIGHT_HOST_CGROUP_MEMORY_MODE"] = mode
            else:
                env.pop("IRLIGHT_HOST_CGROUP_MEMORY_MODE", None)

            if configure_paths:
                env["IRLIGHT_CGROUP_MEMORY_CURRENT_PATH"] = str(current_path)
                env["IRLIGHT_CGROUP_MEMORY_MAX_PATH"] = str(maximum_path)
            else:
                env.pop("IRLIGHT_CGROUP_MEMORY_CURRENT_PATH", None)
                env.pop("IRLIGHT_CGROUP_MEMORY_MAX_PATH", None)
    ...
    def test_default_output_contract_is_unchanged(self) -> None:
        ...
        self.assertNotIn("cgroup_memory_status", result.stdout)

    def test_enabled_below_warning_is_ok(self) -> None:
        result = self._run(mode="enabled", current="79")
        self.assertEqual(result.returncode, 0)
        self.assertIn("status=OK", result.stdout)
        self.assertIn("cgroup_memory_status=OK", result.stdout)
    ...
    def test_enabled_at_warning_is_warning(self) -> None:
        ...
        self.assertIn("cgroup_memory_status=WARNING", result.stdout)
    ...
    def test_enabled_at_critical_is_critical(self) -> None:
        ...
        self.assertIn("cgroup_memory_status=CRITICAL", result.stdout)
    ...
    def test_enabled_zero_limit_is_critical(self) -> None:
    ...
    def test_enabled_malformed_input_is_unknown(self) -> None:
        ...
        self.assertIn("cgroup_memory_status=UNKNOWN", result.stdout)
    ...
    def test_invalid_mode_fails_closed_before_components(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_HOST_PRESSURE status=UNKNOWN reason=invalid_cgroup_memory_mode",
        )
