...
CGROUP_WRAPPER = ROOT / "scripts" / "check-host-cgroup-swap-pressure.sh"
CGROUP_CHECKER = ROOT / "scripts" / "check-cgroup-swap-pressure.sh"
SCALAR_HELPER = ROOT / "scripts" / "lib" / "scalar-pressure-common.sh"


class HostCgroupSwapAggregateTest(unittest.TestCase):
    def _run(
        self,
        *,
        mode: str | None = None,
        ...
        configure_max_path: bool = True,
    ) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory(prefix="irlight-host-cgroup-swap-aggregate-") as temporary:
            root = Path(temporary)
            scripts = root / "scripts"
            lib = scripts / "lib"
            lib.mkdir(parents=True)
            shutil.copyfile(SCRIPT, scripts / "check-host-pressure.sh")
            shutil.copyfile(CGROUP_WRAPPER, scripts / "check-host-cgroup-swap-pressure.sh")
            shutil.copyfile(CGROUP_CHECKER, scripts / "check-cgroup-swap-pressure.sh")
            ...
            current_path = root / "memory.swap.current"
            maximum_path = root / "memory.swap.max"
            ...
            if mode is not None:
                env["IRLIGHT_HOST_CGROUP_SWAP_MODE"] = mode
            else:
                env.pop("IRLIGHT_HOST_CGROUP_SWAP_MODE", None)

            if configure_current_path:
                env["IRLIGHT_CGROUP_SWAP_CURRENT_PATH"] = str(current_path)
            else:
                env.pop("IRLIGHT_CGROUP_SWAP_CURRENT_PATH", None)
            if configure_max_path:
                env["IRLIGHT_CGROUP_SWAP_MAX_PATH"] = str(maximum_path)
            else:
                env.pop("IRLIGHT_CGROUP_SWAP_MAX_PATH", None)
    ...
    def test_default_output_contract_is_unchanged(self) -> None:
        ...
        self.assertNotIn("cgroup_swap_status", result.stdout)

    def test_enabled_below_warning_is_ok(self) -> None:
        result = self._run(mode="enabled", current="79")
        self.assertEqual(result.returncode, 0)
        self.assertIn("status=OK", result.stdout)
        self.assertIn("cgroup_swap_status=OK", result.stdout)
    ...
    def test_enabled_at_warning_is_warning(self) -> None:
        ...
        self.assertIn("cgroup_swap_status=WARNING", result.stdout)
    ...
    def test_enabled_at_critical_is_critical(self) -> None:
        ...
        self.assertIn("cgroup_swap_status=CRITICAL", result.stdout)

    def test_enabled_unlimited_maximum_is_ok(self) -> None:
        result = self._run(mode="enabled", current="90", maximum="max")
        self.assertEqual(result.returncode, 0)
        self.assertIn("status=OK", result.stdout)
        self.assertIn("cgroup_swap_status=OK", result.stdout)

    def test_enabled_zero_limit_without_usage_is_ok(self) -> None:
        result = self._run(mode="enabled", current="0", maximum="0")
        self.assertEqual(result.returncode, 0)
        self.assertIn("status=OK", result.stdout)
        self.assertIn("cgroup_swap_status=OK", result.stdout)

    def test_enabled_over_limit_is_critical(self) -> None:
        result = self._run(mode="enabled", current="101", maximum="100")
        self.assertEqual(result.returncode, 2)
        self.assertIn("status=CRITICAL", result.stdout)
        self.assertIn("cgroup_swap_status=CRITICAL", result.stdout)
    ...
    def test_enabled_malformed_input_is_unknown(self) -> None:
        ...
        self.assertIn("cgroup_swap_status=UNKNOWN", result.stdout)
    ...
    def test_enabled_without_max_target_is_unknown(self) -> None:
        result = self._run(mode="enabled", configure_max_path=False)
        self.assertEqual(result.returncode, 3)
        self.assertIn("status=UNKNOWN", result.stdout)
        self.assertIn("cgroup_swap_status=UNKNOWN", result.stdout)
    ...
    def test_invalid_mode_fails_closed_before_components(self) -> None:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "IRLIGHT_HOST_PRESSURE status=UNKNOWN reason=invalid_cgroup_swap_mode",
        )
