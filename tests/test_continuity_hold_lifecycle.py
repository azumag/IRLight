...
class ContinuityHoldLifecycleTest(unittest.TestCase):
    ...
    def test_missing_hold_deadline_is_recovered_from_last_ingest_time(self) -> None:
        session_id = self._holding_session(last_ingest_at=100.0)

        result = Reaper(self.store, self.provider, self.config, now=110.0).run()

        self.assertEqual(result["hold_deadlines_recovered"], 1)
        self.assertEqual(result["deadline_stops"], 0)
        session = self.store.get(session_id)
        assert session is not None
        self.assertEqual(session["status"], "HOLDING")
        self.assertEqual(session["hold_deadline_at"], 130.0)
        event = session["events"][-1]
        self.assertEqual(event["type"], "session.holding")
        self.assertEqual(event["reason_code"], "INGEST_DISCONNECTED")
        self.assertEqual(event["payload"]["hold_deadline_at"], 130.0)
    ...
    def test_recovered_deadline_survives_store_restart_without_extending_window(self) -> None:
        ...
        self.assertEqual(session["status"], "HOLDING")
        self.assertEqual(
            [e["type"] for e in session["events"]].count("session.holding"),
            1,
        )
    ...
    def test_hold_timeout_finishes_session_with_auditable_reason(self) -> None:
        ...
        self.assertFalse(session["entitlement_reserved"])
        tail = session["events"][-2:]
        self.assertEqual([event["type"] for event in tail], ["session.stopping", "session.finished"])
        self.assertTrue(all(event["reason_code"] == "HOLD_TIMEOUT" for event in tail))
        self.assertEqual(tail[0]["payload"]["from_state"], "HOLDING")
    ...
    def test_timeout_stop_wins_over_late_ingest_observation(self) -> None:
        ...
        self.assertEqual(before["status"], "FINISHED")
    ...
    def test_ready_timeout_uses_distinct_reason_code(self) -> None:
        ...
        self.assertEqual(finished["events"][-2]["type"], "session.stopping")
        self.assertEqual(finished["events"][-2]["reason_code"], "NO_INGEST_TIMEOUT")
        self.assertEqual(finished["events"][-1]["type"], "session.finished")
