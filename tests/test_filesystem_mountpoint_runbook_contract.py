...
class FilesystemMountpointRunbookContractTest(unittest.TestCase):
    ...
    def test_runbook_keeps_read_only_fail_closed_contract(self) -> None:
        ...
        for marker in (
            ...
            "IRLIGHT_EXPECTED_MOUNT_ROOT",
            "status=CRITICAL reason=mount_identity_mismatch",
            "status=UNKNOWN reason=mountpoint_ambiguous",
            "status=UNKNOWN reason=invalid_expected_mount_identity",
            "provider volume",
            "volume generation",
