...
class NetworkEgressTcpSnmpTest(unittest.TestCase):
    def run_check(
        self,
        *,
        mode: str | None = None,
        ...
        with tempfile.TemporaryDirectory(prefix="irlight-network-egress-tcp-") as tmp:
            root = Path(tmp)
            scripts = root / "scripts"
            scripts.mkdir()
            ...
            for name in (
                ...
                target = scripts / name
                if name == "check-tcp-snmp-retransmits.sh" and tcp_checker_body is not None:
            ...
            if mode is not None:
                env["IRLIGHT_TCP_SNMP_RETRANSMITS_MODE"] = mode
    ...
    def test_enabled_ok_and_warning_report_tcp_field(self) -> None:
        baseline = snmp_record(OutSegs=100, RetransSegs=3)
        ok, _ = self.run_check(mode="enabled", current=baseline, baseline=baseline)
        self.assertEqual(ok.returncode, 0)
        self.assertEqual(
            ok.stdout.strip(),
            "IRLIGHT_NETWORK_EGRESS_HEALTH status=OK link_status=OK "
            "ipv4_route_status=OK ipv6_route_status=OK tcp_snmp_retransmits_status=OK family=dual",
        )

        warning, _ = self.run_check(
            mode="enabled",
            current=snmp_record(OutSegs=110, RetransSegs=4),
            baseline=baseline,
        )
        self.assertEqual(warning.returncode, 1)
        self.assertIn("status=WARNING", warning.stdout)
        self.assertIn("tcp_snmp_retransmits_status=WARNING", warning.stdout)

    def test_missing_baseline_and_invalid_mode_fail_closed(self) -> None:
        missing, _ = self.run_check(mode="enabled")
        self.assertEqual(missing.returncode, 3)
        self.assertIn("tcp_snmp_retransmits_status=UNKNOWN", missing.stdout)

        invalid, _ = self.run_check(mode="yes")
        self.assertEqual(invalid.returncode, 3)
        self.assertIn("tcp_snmp_retransmits_status=UNKNOWN", invalid.stdout)

    def test_confirmed_route_critical_wins_over_tcp_warning_or_unknown(self) -> None:
        warning, _ = self.run_check(
            mode="enabled",
            current=snmp_record(OutSegs=1, RetransSegs=1),
        ...
        self.assertIn("tcp_snmp_retransmits_status=UNKNOWN", unknown.stdout)
    ...
    def test_all_optional_components_have_stable_order_and_severity(self) -> None:
        ...
        self.assertEqual(
            ...
            "udp_snmp_errors_status=WARNING tcp_snmp_retransmits_status=WARNING family=dual",
        )

    def test_tcp_component_timeout_becomes_unknown(self) -> None:
        ...
        self.assertIn("tcp_snmp_retransmits_status=UNKNOWN", result.stdout)
