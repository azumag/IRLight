...
SCRIPT = ROOT / "scripts" / "smoke-srt-netem-degradation-matrix.sh"
CORE = ROOT / "scripts" / "smoke-srt-netem-degradation-matrix-core.sh"
LATENCY_HARNESS = ROOT / "scripts" / "smoke-netem-latency-levels.sh"
MATRIX_WORKFLOW = ROOT / ".github" / "workflows" / "srt-netem-degradation-matrix.yml"
LATENCY_WORKFLOW = ROOT / ".github" / "workflows" / "netem-latency-levels.yml"


class SrtNetemDegradationMatrixSmokeIsolationTest(unittest.TestCase):
    @classmethod
    ...
    def test_public_wrapper_quarantines_all_core_output(self) -> None:
        ...
        self.assertIn("srt-netem-matrix-quarantined", self.source)
    ...
    def test_ci_entrypoints_do_not_bypass_public_wrapper(self) -> None:
        for workflow in (self.matrix_workflow, self.latency_workflow):
            self.assertIn('"scripts/smoke-srt-netem-degradation-matrix-core.sh"', workflow)
        self.assertIn(
            "bash ./scripts/smoke-srt-netem-degradation-matrix.sh",
            self.matrix_workflow,
        )
        self.assertNotIn(
            "bash ./scripts/smoke-srt-netem-degradation-matrix-core.sh",
            self.matrix_workflow,
        )

    def test_latency_harness_keeps_generated_srt_core_behind_wrapper(self) -> None:
        self.assertIn(
        ...
        self.assertIn('SRT_NETEM_MATRIX_CORE="$tmp_script"', self.latency_source)
    ...
    def test_core_temporary_material_is_private_and_run_scoped(self) -> None:
        ...
        self.assertIn('override="$tmp_dir/srt-netem-matrix.override.yml"', self.core_source)
