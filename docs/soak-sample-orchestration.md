...
The resource collector resolves each container process to its host PID and reads `/proc/<pid>/statm` plus `/proc/<pid>/fd` so RSS and open-file-descriptor evidence describes the actual workload rather than the collector process. The account running a measured soak therefore needs permission to inspect those host process entries. Container processes are commonly owned by root even when Docker is controlled by an unprivileged account; on such hosts run the soak from an appropriately privileged process-inspection context. The GitHub-hosted Linux evidence-chain smoke uses `sudo` for this reason. This privilege is used only for the read-only disposable-project smoke; the collector itself still refuses non-`irlight-poc-soak-*` projects and does not mutate Docker resources.
