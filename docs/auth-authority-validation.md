...
This document describes the authentication-store slice implemented for #87. Other persisted authority stores still need the same record-level audit before #87 is complete, including explicit schema/enum decisions where compatibility policy is required. This change does not invent new user-status values or constrain the still-open role vocabulary; introducing additional status values requires an explicit migration/API contract. Damaged records are not migrated automatically.
