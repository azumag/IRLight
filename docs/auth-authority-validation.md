...
User records require their persisted identity, normalized email, password hash, role, status, and finite creation/update timestamps. Current writers emit only `status="active"`; any other persisted status is rejected as unsupported authority until an explicit status migration and API contract introduces it. The role vocabulary remains intentionally open, but persisted role strings must already be in canonical form with no leading or trailing whitespace; validation never trims or guesses a repaired role. Persisted emails must also match the registration writer's basic shape: they contain `@`, but do not begin or end with it. `created_at` is non-negative and `updated_at` must not be earlier than `created_at`, matching the writer's monotonic record lifecycle; negative or reversed timestamps are treated as damaged authority instead of being accepted silently. The record key must match the persisted user ID, and `email_index` must point back to the same user record. Optional display names must be strings when present.
...
This document describes the authentication-store slice implemented for #87. Other persisted authority stores still need the same record-level audit before #87 is complete, including explicit schema/enum decisions where compatibility policy is required. This change does not invent new user-status values or constrain the still-open role vocabulary; it only requires any persisted role value to be represented without surrounding whitespace. Introducing additional status values requires an explicit migration/API contract. Damaged records are not migrated automatically.
