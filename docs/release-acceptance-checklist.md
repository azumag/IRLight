...
`node-capacity-load` has an additional semantic gate: when that item is marked `satisfied`, at least one referenced evidence file must pass `scripts/validate-node-capacity-coverage-manifest.py` **and** report `provenance_bound=true`. A schema-v1 complete-coverage manifest remains a valid compatibility artifact for tooling and historical evidence, but it no longer satisfies release acceptance because it binds only the canonical load plan and persisted reports. Release acceptance now requires schema v2, where every scenario also binds the raw trial stream and run manifest and each persisted report is reconstructed from that raw provenance before complete-coverage validation runs. Documentation, individual reports, raw trial files, run manifests, and the plan itself may be supplementary evidence, but none can satisfy the item without a valid provenance-bound complete coverage manifest.

The coverage gate remains intentionally narrow. It verifies the repository evidence graph and the internal consistency of the measured reports and raw run provenance; it does **not** prove that measurements are real, choose a media mix, acceptance threshold or safety margin, approve provider cost, update scheduler inventory, or change production `max_sessions`. Operators must still commit sanitized artifacts from the actual intended Node profile and approved workload rather than fabricating synthetic files merely to satisfy the machine gate.
...
4. For `node-capacity-load`, commit the canonical load plan, the raw trials and run manifest for every required scenario, the canonical persisted report reconstructed from those files, and a schema-v2 coverage manifest that binds the complete provenance set. Reference the schema-v2 coverage manifest in the checklist; schema-v1 coverage remains readable for migration/history but is insufficient for `satisfied` release acceptance.
