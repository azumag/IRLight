...
Bandwidth cases are additive and explicit because #13 does not define canonical rate values. Supply one or more bounded values with repeatable `--bandwidth-kbit` arguments:
...
For both protocols this adds stable IDs such as `rtmp-bandwidth-2500kbit-30s` and `srt-bandwidth-4000kbit-30s` without changing the baseline inventory. Duplicate rate arguments are deduplicated in first-seen order; values outside 64..100000 kbit/s fail closed. The selected profile duration applies to explicit bandwidth cases as well as loss and latency cases.

The baseline still does not invent discrete jitter values because #13 names jitter without defining a numeric matrix. Explicit jitter cases remain available through `network-fault-injector.py plan`. Burst loss, TCP reset, DNS failure, and route mutation remain separate follow-up slices with their own cleanup semantics.
...
The matrix regression verifies the RTMP/SRT baseline inventory, stable unique IDs, additive and deduplicated explicit bandwidth cases, bandwidth bounds, bounded duration behavior, namespace/interface safety reuse, protocol filtering, and that manifest generation never invokes a subprocess.

The case-runner regression verifies stable case selection, explicit bandwidth selection and regeneration, read-only planning, exact regenerated-case matching, rejection of tampered argv/bandwidth metadata/undeclared fields, explicit programmatic disposable-namespace and loopback acknowledgements, namespaced execution, command cleanup after normal operation/apply failure/interrupt, cleanup-failure visibility, and invalid-case rejection.
