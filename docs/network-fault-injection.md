...
Use repeated `--protocol rtmp` / `--protocol srt` arguments to select a subset, and `--duration` to choose one of the existing bounded durations for loss, latency, explicit jitter, and explicit bandwidth profiles. Complete-disconnect cases always retain the full duration matrix required by #13.
...
The selected profile duration applies to explicit jitter cases. The manifest records the explicit pairs in `jitter_profiles`, and each case records `jitter_ms` in its closed fault mapping. Because this changes the machine-readable manifest shape beyond the bandwidth-only contract, the manifest schema version is 3.
