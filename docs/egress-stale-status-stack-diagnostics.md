...
The fingerprint is bounded to at most four threads, eight retained frames per thread, 1024 output bytes, and the newest 256 KiB of input. The extractor streams through the scenario log with a fixed-size tail because the SIGUSR2 traceback is requested at the failure boundary and is therefore expected near the end of the log; this preserves the most relevant recent evidence without making memory usage unbounded. `capped=yes` means the durable fingerprint is not known to represent the complete available evidence: older input bytes were discarded, a frame/output bound was reached, or the extractor itself failed and the wrapper emitted its fixed fail-closed fallback. `capped=no` is reserved for extractor results produced from the complete retained input; unexpected syntax or a complete trace without an allowlisted frame becomes the fixed value `stack_fingerprint=UNAVAILABLE capped=no`. Only this fixed-format fingerprint is added to the step summary that CI already uploads as the short-lived Docker failure artifact. Passing smokes produce no fingerprint or extra artifact noise.
