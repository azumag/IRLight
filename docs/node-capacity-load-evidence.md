...
Before append, the recorder validates the new trial with the same canonical trial schema used by the report validator. Under an advisory sidecar lock (`capacity-trials.jsonl.lock`), it opens or creates the raw JSONL once without following a final symlink, verifies the regular-file device/inode, and keeps that same file descriptor pinned while it reads, parses, validates, checks the final record boundary, appends, flushes, and `fsync`s. The public path must still name that pinned inode immediately before and after publication, and the file size must still match the validated snapshot before the append. This prevents a path replacement between validation and append from redirecting the new trial into a different file. A malformed existing file, unexpected replacement, invalid sequence, evidence file above 1 MiB, or an append that would cross the 1 MiB boundary fails closed without adding the candidate trial. The canonical 4,096-trial limit is applied before publication as well. New evidence and lock files are created with mode `0600`, and symbolic-link evidence targets are refused. When the recorder creates a new evidence file, it also `fsync`s the parent directory after the first record itself has been synced so a successful first append includes the new pathname under normal POSIX durability semantics. Existing-file appends do not need a directory sync because they do not change the namespace.

If an append attempt fails after writing any bytes, the recorder truncates the pinned inode back to the validated pre-append size and `fsync`s that rollback. If the target was created by that failed attempt and is still the same empty inode afterward, the recorder removes it so a transient write failure does not leave an invalid empty run file behind, and the parent directory is synced after that removal. If the first record was fully written but the new pathname cannot be durably synced, the recorder rolls the file back to zero before applying the same identity-checked removal path rather than reporting a successful but unsynced creation. A rollback or cleanup-sync failure is reported together with the original recording failure rather than silently treating the evidence as valid. These steps harden ordinary runtime write/`fsync` failures and successful new-file publication, but they are still not a transactional guarantee across an abrupt process/host loss in the middle of an append; non-cooperating writers also remain outside the guarantee.
