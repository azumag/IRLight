...
The runner is read-only with respect to the Compose project. It delegates every observation to the existing collector and therefore only inspects a disposable project whose name matches `irlight-poc-soak-*`. It never executes `docker compose up`, `stop`, `restart`, `down`, `rm`, volume deletion, provider operations, or external-platform operations.

The output path is created with mode `0600` and must not already exist. The runner refuses to overwrite prior evidence. Every validated sample is written as one compact JSON object, flushed, and `fsync`ed before the next interval. If collection later fails or the operator interrupts the run, previously completed lines remain available as partial failure evidence.
...
The baseline is recorded with `elapsed_seconds=0`. Subsequent samples use actual monotonic elapsed time rather than pretending a delayed observation happened at its nominal schedule. A final observation is always scheduled for the requested duration even when the interval does not divide it exactly.
...
- A child collector timeout or invalid/multi-line/non-finite JSON fails the run closed.
