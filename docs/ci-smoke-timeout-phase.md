...
`scripts/smoke-egress-reconnect.sh` emits a fixed-vocabulary marker before each major blocking phase:

```text
IRLIGHT_DOCKER_SMOKE_PHASE phase=<token>
```

These markers contain only hard-coded ASCII tokens. They never contain a destination URL, stream key, raw status, container log, command line, or arbitrary external input.
...
This diagnostic does not extend the 180-second scenario bound, the legacy 45-second `RECONNECTING` contract, or any runtime retry/stall policy. It only makes outer-timeout evidence durable enough to distinguish which major reconnect-smoke phase was active when the process was terminated.
