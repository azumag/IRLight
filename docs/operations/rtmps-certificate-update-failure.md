...
This runbook covers the operational case where the certificate used by the Media Node RTMPS listener is missing, invalid, expired, or close to expiry. It is intentionally read-only until an operator explicitly replaces certificate files. It does not renew certificates, restart active Sessions, change DNS, or contact a certificate authority.
...
The checker reads only the public certificate and emits a small JSON record. It never reads `NODE_RTMPS_KEY_FILE`, prints the certificate path, or opens a network connection.

Exit codes and fixed reason codes are:

| Exit | Status | Code | Meaning |
| --- | --- | --- | --- |
| `0` | `OK` | `RTMPS_CERT_VALID` | Certificate parses and remains valid beyond the configured window. |
...
5. whether the failure is certificate expiry, hostname/SAN mismatch, key mismatch, incomplete chain, or a client trust-store problem.
...
3. validate expiry, hostname/SAN, and key/certificate match;
...
- checker status/code and `not_after`;
