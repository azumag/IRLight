...
| TCP socket memory pressure | `IRLIGHT_HOST_TCP_MEMORY_MODE` | `tcp_memory_status` | `IRLIGHT_TCP_SOCKSTAT_PATH` / `IRLIGHT_TCP_MEM_PATH` | — | baseline 不要。kernel の `tcp_mem` pressure/max watermark を read-only で評価し、repository 独自 threshold は導入しない | [tcp-memory-pressure-check.md](tcp-memory-pressure-check.md) |
