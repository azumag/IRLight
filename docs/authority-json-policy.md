...
The Continuity service treats that same `control.json` as an authority input rather than permissive application JSON. Its isolated runtime reader rejects duplicate object keys and the non-standard `NaN` / `Infinity` constants before schema validation, and rejects integer timestamps that overflow float normalization instead of allowing an uncaught `OverflowError` to escape the reconcile path. An invalid read preserves the last valid command; before any valid command has been observed the existing fail-safe remains `MUTED`. The Continuity image keeps this small decoder guard local rather than importing Control Plane runtime modules, so its current packaging and `PYTHONPATH=/app` boundary do not change.
